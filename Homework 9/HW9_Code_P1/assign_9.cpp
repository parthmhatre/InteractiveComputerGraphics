...
vector <vec2> texture_values;
void buffer_function (vec4, int, vec3, int, vec2, int);
void display(void);
void start();
...
void values_into_vertices_faces() {
    ...
    while(m<u){
        ...
        while(n<v){
            ...
            texture_values.push_back(vec2(a, b));
            b=b+dv;
            if(b>1)
                b=1;
            n++;
...
void quad( int a, int b, int c ){
    ...
    normalized_values[a] += temp;
    normalized_values[b] += temp;
    normalized_values[c] += temp;

    points.push_back(vertices[a]);
    points.push_back(vertices[b]);
    points.push_back(vertices[c]);

    tex_coords.push_back(texture_values[a]);
    tex_coords.push_back(texture_values[b]);
    tex_coords.push_back(texture_values[c]);
...
void start () {
    ...
    for (int i=0; i<faces.size(); i++){
        ...
        colors.push_back(vec3(temp_normal1.x, temp_normal1.y, temp_normal1.z));

        temp_normal2 = normalize(normalized_values[(int)faces[i].y-1]);
        colors.push_back(vec3(temp_normal2.x, temp_normal2.y, temp_normal2.z));

        temp_normal3 = normalize(normalized_values[(int)faces[i].z-1]);
        colors.push_back(vec3(temp_normal3.x, temp_normal3.y, temp_normal3.z));
    }
...
//----------------------------------------------------------------------------
void buffer_function (vec4 value[], int value_size, vec3 value_color[], int value_color_size, vec2 arr3[], int arr3_size ) {
    ...
    GLuint buffer_text;
    glGenBuffers( 1, &buffer_text );
    glBindBuffer( GL_ARRAY_BUFFER, buffer_text );
    glBufferData( GL_ARRAY_BUFFER, arr3_size, arr3, GL_STATIC_DRAW );

    GLuint buffer_texture = glGetAttribLocation( program, "buffer_texture" );
    glEnableVertexAttribArray( buffer_texture );
    glVertexAttribPointer( buffer_texture, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0) );
...
void display( void ) {
    ...
    vec2 arr3[tex_coords.size()];
    std::copy(tex_coords.begin(), tex_coords.end(), arr3);

    buffer_function ( arr1, sizeof(arr1), arr2, sizeof(arr2), arr3, sizeof(arr3) );
